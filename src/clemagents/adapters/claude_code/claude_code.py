...
from ..utils import (GAME_MCP_SERVER_NAME, configure_tls, load_model_connection, model_connection_environment,
                     new_game_completion_path, read_game_completion, require_resolved_tls, resolve_runtime_model,
...
from .parse import parse_claude_code_agent_trace
from .utils import _anthropic_proxy_base_url, _resolve_model_connection
...
class ClaudeCodeHarness(ExternalAgentHarness):
    ...
    def __init__(self, model: str | None = None, clem_model: str | None = None,
        ...
        Args:
            ...
            reasoning_effort: model reasoning effort passed to Claude Code;
                ``none``/``off`` requests native thinking disablement, whose
        ...
        self.allowed_tools = allowed_tools or [f"mcp__{GAME_MCP_SERVER_NAME}__start_game",
    ...
    async def run_episode_async(self, instruction: str, runtime_model: str | None,
        ...
        options_kwargs = {"mcp_servers": {GAME_MCP_SERVER_NAME: {"type": "stdio",
                                                                 "command": "python",
        ...
        if self.reasoning_effort in {"none", "off"}:
    ...
    def _run_episode(self, instruction: str, output_dir: Path | str | None, completion_path: Path,
        ...
        runtime_model = resolve_runtime_model(model_connection=self._model_connection, model=self.model,
                                              harness_name="ClaudeCodeHarness", required=False)
        ...
        coroutine = self.run_episode_async(instruction=instruction, runtime_model=runtime_model,
                                           runtime_environment=runtime_environment, completion_path=completion_path)
        ...
        completion = read_game_completion(completion_path)
        game_completed = bool(game_completed or (completion and completion.get("done") is True
        ...
        artifacts = {}
        messages_path = write_text_artifact(output_dir=output_dir, filename="adapter_messages.txt",
