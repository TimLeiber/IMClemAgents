...
def parse_codex_agent_trace(episode_dir: Path,
    ...
    events.extend(_codex_runtime_errors(
        trace_text,
        final_turn=model_response_count or None,
    ))
...
def _codex_response_events(record: dict[str, Any],
    ...
    for line in record["raw"].splitlines():
        ...
        if event_type == "response.output_item.added":
            ...
            terminal_failure = (
                ...
                or {"message": "Provider response did not complete"}
            )
    ...
    function_calls = [item for item in completed_items
                      if item.get("type") == "function_call"]
    completed_items = [item for item in completed_items
                       if item.get("type") != "function_call"] + function_calls
    ...
    incomplete_tool_items = [
        item
        for item_id, item in started_tool_items.items()
...
def _codex_runtime_errors(trace_text: str,
                          final_turn: int | None = None) -> list[dict[str, Any]]:
