...
@contextmanager
def _openclaw_gateway(base_command: list[str], log_path: Path,
        ...
        try:
            ...
            while time.monotonic() < deadline:
                if process.poll() is not None:
                    raise RuntimeError(
                        f"OpenClaw Gateway exited during startup ({process.returncode}); "
                        f"see {log_path.name}"
                    )
                try:
                ...
                raise RuntimeError(
                    f"OpenClaw Gateway was not ready after {startup_timeout}s; "
                    f"see {log_path.name}"
                )
            yield
        finally:
...
class OpenClawHarness(ExternalAgentHarness):
    ...
    def _run_episode(self,
        ...
        run_dir = (Path(output_dir) if output_dir is not None
                   else Path("/tmp") / f"openclaw-episode-{os.getpid()}")
        ...
        session_parts = (
            "game-session",
            os.environ.get("GAME_EXPERIMENT", "experiment"),
            os.environ.get("GAME_INSTANCE_ID", "game"),
            str(os.getpid()),
        )
        session_key = "agent:main:" + "-".join(
        ...
        if not (plugin_path / "openclaw.plugin.json").is_file():
            metadata["runtime_error"] = (
                "OpenClaw's native DuckDuckGo plugin is missing; rebuild "
        ...
        mcp_command = base_command + [
            "mcp", "add", GAME_MCP_SERVER_NAME,
            "--command", "python",
            "--arg", "-m",
            "--arg", "clemagents.mcp.bridge",
            "--cwd", "/opt/clemagents/src",
        ...
        config = deep_merge_dicts(config, {
            ...
            # The Gateway supplies tools, not independent background model turns.
        ...
        if gateway_log_path.exists():
            gateway_log_path.write_text(
                redact_sensitive(gateway_log_path.read_text(encoding="utf-8", errors="replace")),
                encoding="utf-8",
            )
    ...
    if backend == "openai_compatible":
        ...
        return {
            **common,
            ...
            "env": {
                api_key_env: connections._openai_compatible_api_key(key_config),
            },
