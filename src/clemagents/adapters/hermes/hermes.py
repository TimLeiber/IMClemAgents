...
from ..utils import (GAME_MCP_SERVER_NAME, configure_tls, load_model_connection, mcp_environment,
                     ...
                     temporary_environment, warn_model_generation_config, write_ca_bundle, write_text_artifact)
from ..utils.openai_compatible_proxy import proxy_for_model_connection
from . import utils
from .parse import parse_hermes_agent_trace
from .utils import _check_setup, _export_session, _resolve_model_connection, _session_id
...
class HermesHarness(ExternalAgentHarness):
    ...
    def _run_episode(self, instruction: str, output_dir: Path | str | None, completion_path: Path,
        ...
        try:
            ...
            environment.update(utils.configure(home, config, observer_path, completion_path,
                                                          self.reasoning_effort, self.trace_model_io))
            with temporary_environment(environment):
                _check_setup(trace_parts)
                ...
                session = _session_id(observer_path, chat.stdout + "\n" + chat.stderr)
                metadata["hermes_session_id"] = session
                if session:
                    _export_session(session, run_dir, artifacts, trace_parts)
