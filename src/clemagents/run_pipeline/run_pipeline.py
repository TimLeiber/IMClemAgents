...
from .utils import (DOCKER_IMAGE,  # docker image used for agent episodes
                    ...
                    _write_run_registry,  # pass run settings to the adapter
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Run external agent harnesses through clembench/OpenEnv/MCP.")

    # define the pipeline arguments
    parser.add_argument("-g", "--game", required=True, help="Game name, matching clem run -g.")
    parser.add_argument("-a", "--agent", required=True, help="External agent name from the agent registry.")
    parser.add_argument("-m", "--models", nargs="+", default=[],
                        help="Native clem model(s) for non-external players, matching clem run -m style.")
    parser.add_argument("--agent-player", default="player_0",
    ...
    parser.add_argument("-i", "--instances_filename", default=None,
                        help="Instances file name without .json, matching clem run -i.")
    parser.add_argument("-r", "--results_dir", default="results/external-agents",
                        help="Results root directory, matching clem run -r style.")
    parser.add_argument("-t", "--temperature", type=float, default=None,
                        help=("Native adapter temperature and native environment-model temperature. "
                              "Requires adapter support; never injected into API requests. "
                              "Omitted keeps agent configuration and uses 0.0 for native models."))
    parser.add_argument("-l", "--max_tokens", type=int, default=300,
    ...
    env_agents = _env_agents_from_models(models=args.models, learner_agent=args.agent_player, game_name=args.game)

    # main step 1
    # load and filter the selected game instances
    game_instances = load_game_instances(game_name=args.game, instances_filename=args.instances_filename,
    ...
    print(f"temperature: {args.temperature if args.temperature is not None else '<agent configuration>'}")
    print(f"episode_timeout: {args.episode_timeout:g} seconds")
    print(game_instances.describe())

    result_run_dir = result_run_dir_name(agent_name=args.agent, registry_path=REGISTRY_PATH,
                                         learner_agent=args.agent_player, env_agents=env_agents)

    # create temporary shared storage for the agent runtime
    temp_dir = tempfile.TemporaryDirectory(prefix="clem-agent-model-")
    agent_settings = {"temperature": args.temperature} if args.temperature is not None else {}
    run_registry = _write_run_registry(args.agent, Path(temp_dir.name), agent_settings)
    # resolve and write the external agent model connection
    model_connection_path = _write_agent_model_connection(agent_name=args.agent, output_dir=Path(temp_dir.name),
    ...
    server_process = start_server(game_name=args.game, agent_name=args.agent, agent_player=args.agent_player,
                                  env_agent_models=args.models, gen_args={"temperature": native_temperature,
                                                                          "max_tokens": args.max_tokens}, instances_filename=args.instances_filename,
                                  results_dir=args.results_dir, completion_path=game_completion_path)
    ...
    try:
        ...
        for row in tqdm(selected_instances, desc="Playing game instances"):
            ...
            before_instance_dirs = {path
                                    for path in Path(args.results_dir).glob(f"{result_run_dir}/{args.game}/{experiment_name}/instance_*")
                                    if path.is_dir()}
            ...
            trace_text, return_code, runtime_metadata = run_docker_episode(experiment_name=experiment_name,
                                                                           game_id=game_id, agent_name=args.agent,
                                                                           model_connection_path=model_connection_path,
                                                                           shared_state_dir=Path(temp_dir.name),
                                                                           episode_timeout=args.episode_timeout,
                                                                           registry_path=run_registry)
            artifact_relative_path = runtime_metadata.get("agent_artifact_relative_path")
            source_artifact_dir = (Path(temp_dir.name) /
                                   artifact_relative_path if isinstance(artifact_relative_path, str) else None)
            ...
            write_agent_artifacts(results_dir=args.results_dir, run_dir=result_run_dir, game_name=args.game,
                                  experiment_name=experiment_name, game_id=game_id,
                                  before_instance_dirs=before_instance_dirs, trace_text=trace_text, metadata=metadata,
