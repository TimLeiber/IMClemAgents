...
AGENT_TERMINATION_REASON = (
    "AGENT_CONTROL_ERROR: external harness ended before completing the game"
)
EPISODE_TIMEOUT_REASON = (
    "AGENT_EPISODE_TIMEOUT: host wall-clock limit reached"
)
ARTIFACT_FINALIZATION_TIMEOUT = 90.0
ARTIFACT_EXIT_GRACE = 2.0


def _run_mcp_server_process(server_kwargs: dict,
...
def _drain_server_startup_status(startup_status_queue,
...
def _format_server_startup_status(statuses: list[dict]) -> str:
    ...
    return "Last MCP-server startup status: " + json.dumps(
        statuses[-1],
        ensure_ascii=False,
    )


def _stop_docker_container(process: subprocess.Popen,
...
def _shared_artifact_directory(shared_state_dir: Path | None,
                               marker: dict | None) -> Path | None:
...
def _abort_unfinished_episode(shared_state_dir: Path | None,
                              experiment_name: str,
                              game_id: int | str,
    ...
    try:
        if session_path.exists():
            ...
            trace_lines.append(
                "host_game_abort_failed: game was started but no open session marker exists\n"
            )
            return None
...
def start_server(game_name: str,
                 ...
                 results_dir: str,
                 completion_path: Path | None = None) -> Process:
    ...
    while time.monotonic() < deadline:
        ...
        if not process.is_alive():
            ...
            _drain_server_startup_status(startup_status_queue, startup_statuses)
            raise RuntimeError(
                "MCP server process exited during startup. "
...
def run_docker_episode(experiment_name: str,
                        ...
                        artifact_exit_grace: float = ARTIFACT_EXIT_GRACE) -> tuple[str, int, dict]:
    ...
    command = [
        ...
        "-e", f"GAME_EXPERIMENT={experiment_name}",
        "-e", f"GAME_INSTANCE_ID={game_id}",
        "-e", f"AGENT_NAME={agent_name}",
        "-e", "GAME_OBSERVATION_DIR=/workspace/game_observations",
        "-v", f"{PACKAGE_ROOT}:/opt/clemagents/src/clemagents:ro",
    ...
    if shared_state_dir is not None:
        ...
        command.extend([
            "-e", "GAME_SESSION_PATH=/run/game-agent/openenv_session.json",
            "-e", "GAME_STARTED_PATH=/run/game-agent/game_started.json",
            "-e", "AGENT_ARTIFACT_ROOT=/run/game-agent/artifacts",
            "-e", "AGENT_ARTIFACTS_READY_PATH=/run/game-agent/artifacts_ready.json",
            "-e", "AGENT_LIVE_TRACE_PATH=/run/game-agent/live_agent_trace.log",
            "-v", f"{shared_state_dir}:/run/game-agent:rw",
        ])

    # tell the container where to find the resolved model connection
    if model_connection_path is not None:
        command.extend([
            "-e", "AGENT_MODEL_CONNECTION_PATH=/run/game-agent/model_connection.json",
        ])

        #  mount the model connection directly when no shared directory is used
        if shared_state_dir is None:
            command.extend([
                "-v", f"{model_connection_path}:/run/game-agent/model_connection.json:ro",
            ])
    ...
    try:
        ...
        while process.poll() is None or not output_closed:
            ...
            if (host_completion is None
                ...
                if isinstance(completion, dict) and completion.get("done") is True:
                    ...
                    terminal_reason = (
                        "game_abort"
                        if completion_metadata.get("control_failure") is True
                        else "game_done"
                    )
                    trace_lines.append(
                        "host_game_completion_observed: "
            ...
            if host_completion is not None and artifacts_ready_marker is None:
                ...
                if isinstance(ready_marker, dict):
                    ...
                    trace_lines.append(
                        "host_agent_artifacts_ready_observed: "
            ...
            if (host_completion is not None
                    and artifacts_ready_marker is None
                ...
                trace_lines.append(
                    "host_artifact_finalization_timeout: "
            ...
            if (artifacts_ready_marker is not None
                    and artifacts_ready_observed_at is not None
                    and process.poll() is None
                    and now - artifacts_ready_observed_at >= artifact_exit_grace):
                trace_lines.append(
                    "host_container_exit_grace_expired_after_artifacts_ready: "
            ...
            if (host_completion is None
                    and not timed_out
                    and elapsed >= episode_timeout):
                timed_out = True
                terminal_reason = "episode_timeout"
                trace_lines.append(
                    "host_episode_timeout: "
                    f"limit_seconds={episode_timeout:g} elapsed_seconds={elapsed:.3f}\n"
                )

                # finalize the game before stopping Docker so clembench persists
                # every turn observed before the deadline
                host_completion = _abort_unfinished_episode(
                    shared_state_dir=shared_state_dir,
                    experiment_name=experiment_name,
                    game_id=game_id,
                    trace_lines=trace_lines,
                    reason=EPISODE_TIMEOUT_REASON,
                )
                _stop_docker_container(process, cidfile_path)
        ...
        if artifacts_ready_marker is None:
            ...
            if artifacts_ready_marker is not None:
                artifacts_ready_observed_at = time.monotonic()
        if ((timed_out or host_completion is None)
                and live_trace_path is not None
                and live_trace_path.exists()):
            ...
            if partial_trace and partial_trace not in captured_trace:
                trace_lines.extend([
                    "partial_agent_trace_snapshot_start\n",
                    partial_trace,
        ...
        if shared_state_dir is not None:
            ...
            if not completed:
                ...
                host_completion = _abort_unfinished_episode(
                    ...
                    game_id=game_id,
                    trace_lines=trace_lines,
                    reason=AGENT_TERMINATION_REASON,
                )

    container_elapsed_seconds = time.monotonic() - episode_started_at
    episode_elapsed_seconds = (
        completion_observed_at - episode_started_at
        if completion_observed_at is not None
        else container_elapsed_seconds
    )
    artifact_finalization_seconds = (
        artifacts_ready_observed_at - completion_observed_at
        if (artifacts_ready_observed_at is not None
            and completion_observed_at is not None)
        else None
    )
    artifact_directory = _shared_artifact_directory(
        shared_state_dir,
        artifacts_ready_marker,
    )
    episode_metadata = {
        "episode_timeout_seconds": episode_timeout,
        "episode_timed_out": timed_out,
        "episode_elapsed_seconds": episode_elapsed_seconds,
        "container_elapsed_seconds": container_elapsed_seconds,
        "episode_terminal_reason": terminal_reason or "container_exit",
        "host_completion": host_completion,
        "artifact_finalization_timeout_seconds": artifact_finalization_timeout,
        "artifact_finalization_timed_out": artifact_finalization_timed_out,
        "artifact_finalization_seconds": artifact_finalization_seconds,
        "artifacts_ready": bool(
            artifacts_ready_marker
            and artifacts_ready_marker.get("ready") is True
        ),
        "artifacts_ready_status": (
            artifacts_ready_marker.get("status")
            if artifacts_ready_marker is not None
            else "unavailable"
        ),
        "agent_artifact_relative_path": (
            str(artifact_directory.relative_to(shared_state_dir.resolve()))
            if artifact_directory is not None and shared_state_dir is not None
            else None
        ),
    }
...
def write_agent_artifacts(results_dir: str,
    ...
    Args:
        ...
        source_artifact_dir: Optional finalized adapter-artifact directory.
    ...
    # Preserve each finalized native artifact set under its own run directory.
    ...
    if source_artifact_dir is not None:
        ...
        if source_path.is_dir():
            ...
            metadata["native_artifact_directory"] = str(
                copied_artifact_dir.relative_to(output_dir)
            )
    ...
    if isinstance(agent_name, str) and agent_name:
        try:
            ...
            harness_class.serialize_standardized_agent_trace(
...
def append_agent_episode_outcome(results_dir: str | Path,
                                 metadata: dict) -> Path:
...
def _write_agent_model_connection(agent_name: str,
                                  output_dir: Path,
                                  temperature: float | None = None) -> Path | None:
    ...
    Args:
        ...
        temperature: optional protocol-neutral sampling temperature for the
            external model; omitted preserves the provider default
    ...
    if temperature is not None:
        ...
        # An explicit run setting takes precedence over any model-connection
        # default. The generic compatibility proxy applies this field to every
        # supported harness protocol.
        connection["generation_overrides"] = {
            **generation_overrides,
