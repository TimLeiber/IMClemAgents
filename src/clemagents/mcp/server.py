...
class _SuppressFastMCPToolCallErrors(logging.Filter):
    """Keep model-authored tool errors out of the host terminal."""

    def filter(self, record: logging.LogRecord) -> bool:
        return not record.getMessage().startswith("Error calling tool ")


def _configure_server_logging(quiet: bool) -> None:
    """Configure concise server logs without hiding MCP errors from clients."""
...
def _report_startup_status(startup_status_queue, state: str, **details) -> None:
    ...
    try:
        startup_status_queue.put_nowait({"state": state, **details})
    except Exception:
        pass
...
def run_clem_mcp_server(game_name: str, agent_name: str, registry_path: str | Path, learner_agent: str = "player_0",
    ...
    Args:
        ...
        startup_status_queue: optional queue for reporting server-startup state
        port: port used by the MCP server
        quiet: whether to suppress informational server logging
        """

    _report_startup_status(startup_status_queue, "initializing", port=port)
