...
from clemagents.run_pipeline.utils import (_stop_docker_container, run_docker_episode, write_agent_artifacts)
from clemagents.adapters.utils import (mcp_environment, run_process_until_game_complete)
...
class TestExternalAgentPipeline(unittest.TestCase):
    ...
    def test_mcp_environment_forwards_generic_observation_directory(self):
        with patch.dict("os.environ", {"GAME_OBSERVATION_DIR": "/workspace/game_observations"}):
            environment = mcp_environment("http://host.example/mcp")

        self.assertEqual(environment["GAME_OBSERVATION_DIR"], "/workspace/game_observations")

    def test_fastmcp_tool_call_errors_are_hidden_from_host_log(self):
        log_filter = _SuppressFastMCPToolCallErrors()
        tool_error = logging.LogRecord("fastmcp.server.server", logging.ERROR, "", 0, "Error calling tool %r",
                                       ("submit_response",), None)
        server_error = logging.LogRecord("fastmcp.server.server", logging.ERROR, "", 0, "MCP server failed", (), None)
    ...
    def test_docker_cleanup_contains_unresponsive_docker_commands(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("clemagents.run_pipeline.utils.subprocess.run",
                       side_effect=subprocess.TimeoutExpired(["docker"], 10)) as docker_command:
    ...
    def test_docker_output_replaces_invalid_utf8(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("clemagents.run_pipeline.utils.KEYS_PATH",
                       keys_path), patch("clemagents.run_pipeline.utils.REGISTRY_PATH",
                                         registry_path), patch("clemagents.run_pipeline.utils.subprocess.Popen",
                                                               side_effect=simulated_container):
                trace, returncode, _ = run_docker_episode(experiment_name="test", game_id=1, agent_name="test-agent",
                                                          episode_timeout=5)
    ...
    def test_codex_tool_result_renders_only_standardized_game_message(self):
        ...
        self.assertEqual(_codex_tool_result_text(output), "Initial game message")

    def test_harness_class_is_resolved_from_agent_registry(self):
        with tempfile.TemporaryDirectory() as directory:
            registry_path = Path(directory) / "agent_registry.json"
            registry_path.write_text(json.dumps([{"agent_name": "codex-test",
                                                  "backend": "codex",
                                                  "agent_config": {}}]), encoding="utf-8")
    ...
    def test_final_agent_trace_is_serialized_after_persistence(self):
        trace = "\n".join(["agent_loop_instruction_start", "Play through MCP tools.", "agent_loop_instruction_end",
                           "raw_upstream_request_1_start", "path: /v1/responses",
                           json.dumps({"instructions": "Native instruction",
                                       "input": []}), "raw_upstream_request_1_end", "raw_upstream_response_2_start", "path: /v1/responses",
                           "status: 200", "content_type: application/json", "content_encoding: ",
                           json.dumps({"output": []}), "raw_upstream_response_2_end"])

        with tempfile.TemporaryDirectory() as directory:
            results_dir = Path(directory) / "results"
            instance_dir = (results_dir / "codex-test" / "wordle" / "medium" / "instance_00001")
            instance_dir.mkdir(parents=True)
            (instance_dir / "instance.json").write_text(json.dumps({"game_id": 1}), encoding="utf-8")
            registry_path = Path(directory) / "agent_registry.json"
            registry_path.write_text(json.dumps([{"agent_name": "codex-test",
                                                  "backend": "codex",
                                                  "agent_config": {}}]), encoding="utf-8")

            trace_path = write_agent_artifacts(results_dir=str(results_dir), run_dir="codex-test", game_name="wordle",
                                               experiment_name="medium",
                                               game_id=1, before_instance_dirs=set(), trace_text=trace, metadata={"agent": "codex-test",
                                                                                                                  "started_at": datetime.now(timezone.utc).isoformat()}, registry_path=registry_path)
            agent_loop = json.loads((instance_dir / "agent_loop.json").read_text(encoding="utf-8"))
    ...
    def test_finalized_native_artifacts_are_preserved_without_overwriting_game_files(self):
        with tempfile.TemporaryDirectory() as directory:
            results_dir = Path(directory) / "results"
            instance_dir = (results_dir / "test-agent" / "wordle" / "medium" / "instance_00001")
            instance_dir.mkdir(parents=True)
            (instance_dir / "instance.json").write_text(json.dumps({"game_id": 1}), encoding="utf-8")
            (instance_dir / "interactions.json").write_text("game-owned", encoding="utf-8")
            source_dir = Path(directory) / "finalized-artifacts"
            source_dir.mkdir()
            (source_dir / "native_session.jsonl").write_text("native artifact", encoding="utf-8")
            (source_dir / "interactions.json").write_text("must not overwrite", encoding="utf-8")

            write_agent_artifacts(results_dir=str(results_dir), run_dir="test-agent", game_name="wordle",
                                  experiment_name="medium", game_id=1, before_instance_dirs=set(),
                                  trace_text="host trace",
                                  metadata={"started_at":
                                            datetime.now(timezone.utc).isoformat()}, source_artifact_dir=source_dir)

            self.assertEqual((instance_dir / "agent_artifacts" / "finalized-artifacts" /
                              "native_session.jsonl").read_text(encoding="utf-8"), "native artifact")
            self.assertEqual((instance_dir / "interactions.json").read_text(encoding="utf-8"), "game-owned")
            self.assertEqual((instance_dir / "agent_trace.log").read_text(encoding="utf-8"), "host trace")

    def test_finalized_native_artifacts_preserve_container_only_symlinks(self):
        with tempfile.TemporaryDirectory() as directory:
            results_dir = Path(directory) / "results"
            instance_dir = (results_dir / "test-agent" / "geolocate" / "civic_public" / "instance_00003")
            instance_dir.mkdir(parents=True)
            (instance_dir / "instance.json").write_text(json.dumps({"game_id": 3}), encoding="utf-8")
            source_dir = Path(directory) / "finalized-artifacts"
            source_dir.mkdir()
            source_link = source_dir / "browser-automation"
            source_link.symlink_to("/container-only/plugin-skills/browser-automation", target_is_directory=True)

            write_agent_artifacts(results_dir=str(results_dir), run_dir="test-agent", game_name="geolocate",
                                  experiment_name="civic_public", game_id=3, before_instance_dirs=set(),
                                  trace_text="host trace",
                                  metadata={"started_at":
                                            datetime.now(timezone.utc).isoformat()}, source_artifact_dir=source_dir)

            copied_link = (instance_dir / "agent_artifacts" / "finalized-artifacts" / "browser-automation")
    ...
    def test_codex_trace_orders_reasoning_before_terminal_tool_call(self):
        response_events = [{"type": "response.output_item.done",
                            "item": {"type": "function_call",
                                     "id": "call-item-1",
                                     "call_id": "call-1",
                                     "name": "mcp__clem_game__start_game",
                                     "arguments": "{}"}}, {"type": "response.output_item.done",
                                                           "item": {"type": "reasoning",
                                                                    "id": "reasoning-1",
                                                                    "content": [{"type": "reasoning_text",
                                                                                 "text": "I should start the game."}]}}]
        trace = "\n".join(["raw_upstream_request_1_start", "path: /v1/responses",
                           json.dumps({"instructions": "Native instruction",
                                       "input": []}), "raw_upstream_request_1_end", "raw_upstream_response_2_start", "path: /v1/responses",
                           "status: 200", "content_type: text/event-stream", "content_encoding: ",
                           *[f"data: {json.dumps(event)}" for event in response_events], "raw_upstream_response_2_end"])
        ...
        semantic_events = [event for event in parsed["events"] if event["type"] in {"reasoning", "tool_call"}]
        self.assertEqual([event["type"] for event in semantic_events], ["reasoning", "tool_call"])
        self.assertEqual(semantic_events[0]["content"], "I should start the game.")
        self.assertEqual(semantic_events[1]["name"], "mcp__clem_game__start_game")
    ...
    def test_codex_trace_keeps_failed_provider_tool_attempts(self):
        ...
        def failed_search_response(response_id, item_id, query, reasoning=None):
            ...
            if reasoning is not None:
                response_events.append({"type": "response.output_item.done",
                                        "item": {"type": "reasoning",
                                                 "id": f"reasoning-{response_id}",
                                                 "summary": [{"type": "summary_text",
                                                              "text": reasoning}]}})

            response_events.extend([{"type": "response.output_item.added",
                                     "item": {"type": "web_search_call",
                                              "id": item_id,
                                              "status": "in_progress",
                                              "action": {"type": "search",
                                                         "query": query}}}, {"type": "response.failed",
        ...
        for turn, events in enumerate([failed_search_response("response-1", "search-1", "first query", reasoning="I should search."),
                                       failed_search_response("response-2", "search-2", "second query")], start=1):
            trace_lines.extend([f"raw_upstream_request_{turn}_start", "path: /v1/responses",
                                json.dumps({"instructions": "Native instruction",
                                            "input": []}), f"raw_upstream_request_{turn}_end", f"raw_upstream_response_{turn}_start",
                                "path: /v1/responses", "status: 200", "content_type: text/event-stream", "content_encoding: ",
                                *[f"data: {json.dumps(event)}" for event in events], f"raw_upstream_response_{turn}_end"])

        trace_lines.extend(["codex_stdout_jsonl:",
        ...
        visible_events = [event for event in parsed["events"] if event["type"] in {"reasoning", "tool_call", "tool_result", "error"}]
        self.assertEqual([event["type"] for event in visible_events],
                         ["reasoning", "tool_call", "tool_result", "tool_call", "tool_result", "error"])
        self.assertEqual([event["arguments"] for event in visible_events if event["type"] == "tool_call"],
                         [{"query": "first query"}, {"query": "second query"}])
        self.assertTrue(all(event["status"] == "failed" for event in visible_events
                            if event["type"] in {"tool_call", "tool_result", "error"}))
        self.assertTrue(all("authentication_error" in event["content"] for event in visible_events
                            if event["type"] == "tool_result"))
        self.assertEqual(visible_events[-1]["turn"], 2)

    def test_claude_proxy_base_does_not_duplicate_v1(self):
        self.assertEqual(_anthropic_proxy_base_url("http://127.0.0.1:1234/api/v1"), "http://127.0.0.1:1234/api")
    ...
    def test_claude_code_trace_parser_standardizes_sdk_messages(self):
        trace = "\n".join(["agent_loop_instruction_start", "Play through MCP tools.", "agent_loop_instruction_end",
                           ...
                           "UserMessage(content=[ToolResultBlock(tool_use_id='call-1', content=" + repr(json.dumps({"context": {"role": "user",
                                                                                                                                "content": "Game prompt"},
                                                                                                                    "reward": None,
                                                                                                                    "done": False,
                                                                                                                    "metadata": {"truncated": False}})) + ", is_error=None)])",
    ...
    def test_claude_code_trace_parser_flattens_text_tool_result_blocks(self):
        trace = "\n".join(["AssistantMessage(content=[ToolUseBlock(id='call-1', name='mcp__clem_game__start_game', input={})], message_id='message-1')",
    ...
    def test_claude_code_trace_parser_preserves_subagent_timeline(self):
        trace = "\n".join(["AssistantMessage(content=[ToolUseBlock(id='agent-call', name='Agent', input={'description': 'Solve Wordle'})], message_id='parent-message')",
        ...
        parent_call = next(event for event in parsed["events"] if event.get("call_id") == "agent-call")
        self.assertNotIn("agent", parent_call)

        child_events = [event for event in parsed["events"] if event.get("agent") == "Solve Wordle [agent-1]"]
        ...
        child_result = next(event for event in child_events if event.get("call_id") == "child-call")
        self.assertEqual(child_result["turn"], 2)

    def test_agent_loop_renderer_handles_generic_subagent_events(self):
        agent_loop = {"schema_version":
                      1,
                      "backend":
                      "future_harness",
                      "events": [{"sequence": 1,
        ...
        rendered = build_agent_loop_html(agent_loop, Path("/tmp/future-harness/agent_loop.json"))
    ...
    def test_hermes_trace_parser_standardizes_verbose_cli_output(self):
        trace = "\n".join(["agent_loop_instruction_start", "Play through MCP tools.", "agent_loop_instruction_end",
                           "hermes_chat_command:", "hermes chat --provider openrouter --model test-model --yolo -q <instruction>",
                           "hermes_chat_stdout:", "Query: Play through MCP tools.", "Initializing agent...",
                           "🤖 AI Agent initialized with model: test-model",
                           "🛠️  Final tool selection (2 tools): web_search, mcp__clem_game__start_game", "┌─ Reasoning ─────────┐",
                           "I should start the game.", "└─────────────────────┘", "  📞 Tool 1: mcp__clem_game__start_game([])",
                           "     Args: {}", "  ┊ ⚡ preparing mcp__clem_game__start_game…", "  ✅ Tool 1 completed in 0.10s",
                           "     Result: {\"structuredContent\": {\"context\": {\"role\": \"user\", \"content\": \"Game prompt\"}, \"done\": false}}",
                           "hermes_chat_stderr:", "success: True"])
    ...
    def test_hermes_trace_parser_prefers_session_export(self):
        trace = "\n".join(["agent_loop_instruction_start", "Play through MCP tools.", "agent_loop_instruction_end",
                           "hermes_chat_stdout:", "Query: Play through MCP tools.", "Initializing agent...", "hermes_chat_stderr:",
                           "success: True"])
        session = {"id":
                   "session-1",
                   "model":
                   "test-model",
                   "messages": [{"role": "user",
                                 "content": "Play through MCP tools."}, {"role":
                                                                         "assistant",
                                                                         "content":
                                                                         "I will verify this.",
                                                                         "reasoning":
                                                                         ...
                                                                         "tool_calls": [{"id": "call-1",
                                                                                         "function": {"name": "web_search",
                                                                                                      "arguments": "{\"query\": \"example\"}"}}]}, {"role": "tool",
                                                                                                                                                    "tool_call_id": "call-1",
        ...
        with tempfile.TemporaryDirectory() as directory:
            episode_dir = Path(directory)
            (episode_dir / "agent_trace.log").write_text(trace, encoding="utf-8")
            (episode_dir / "hermes_session_export.jsonl").write_text(json.dumps(session) + "\n", encoding="utf-8")
            parsed = HermesHarness.parse_agent_trace(episode_dir)
        ...
        self.assertEqual(parsed["capture"]["semantic_events"]["source"], "hermes_session_export")

    def test_hermes_trace_parser_recovers_streamed_wire_events(self):
        instruction = "Play through MCP tools."
        request_one = {"messages": [{"role": "system",
                                     "content": "Native Hermes instruction."}, {"role": "user",
                                                                                "content": instruction}]}
        call_id = "call-1"
        response_one = [{"choices": [{"delta": {"role": "assistant",
                                                "reasoning": "I should inspect the game."},
                                      "finish_reason": None}]}, {"choices": [{"delta": {"role":
                                                                                        "assistant",
                                                                                        "tool_calls": [{"index": 0,
                                                                                                        "id": call_id,
                                                                                                        "type": "function",
                                                                                                        "function": {"name": "mcp__game__start_game",
                                                                                                                     "arguments": ""}}]},
                                                                              "finish_reason": None}]}, {"choices": [{"delta": {"role": "assistant",
                                                                                                                                "tool_calls": [{"index": 0,
                                                                                                                                                "function": {"arguments": "{}"}}]},
                                                                                                                      "finish_reason": "tool_calls"}]}]
        request_two = {"messages":
                       request_one["messages"] + [{"role":
                                                   "assistant",
                                                   "reasoning":
                                                   ...
                                                   "tool_calls": [{"id": call_id,
                                                                   ...
                                                                   "function": {"name": "mcp__game__start_game",
                                                                                "arguments": "{}"}}]}, {"role":
                                                                                                        ...
                                                                                                        call_id,
                                                                                                        "name":
                                                                                                        "mcp__game__start_game",
                                                                                                        "content": [{"type": "text",
                                                                                                                     "text": "Game prompt"}, {"type": "image_url",
                                                                                                                                              "image_url": {"url": "data:image/jpeg;base64,AAAA"}}]}]}
        response_two = [{"choices": [{"delta": {"role": "assistant",
                                                "content": "I can answer now."},
                                      "finish_reason": "stop"}]}]
        trace_lines = ["agent_loop_instruction_start", instruction, "agent_loop_instruction_end", "hermes_chat_stdout:",
                       "┌─ Reasoning ─────────┐", "I should inspect the game.", "└─────────────────────┘", "hermes_chat_stderr:"]

        for number, (request, response) in enumerate(((request_one, response_one), (request_two, response_two)),
                                                     start=1):
            trace_lines.extend([f"raw_upstream_request_{number}_start", "path: /api/v1/chat/completions",
                                json.dumps(request), f"raw_upstream_request_{number}_end", f"raw_upstream_response_{number}_start",
                                "path: /api/v1/chat/completions", "status: 200", "content_type: text/event-stream",
                                "content_encoding: identity", *[f"data: {json.dumps(chunk)}\n" for chunk in response], "data: [DONE]\n",
        ...
        semantic_events = [event for event in parsed["events"]
                           if event["type"] in {"reasoning", "tool_call", "tool_result", "assistant_text"}]
        self.assertEqual([event["type"] for event in semantic_events],
        ...
        self.assertEqual(parsed["capture"]["semantic_events"]["source"], "chat_completions_wire")

        with tempfile.TemporaryDirectory() as directory:
            episode_dir = Path(directory)
            (episode_dir / "agent_trace.log"
         ).write_text("\n".join(trace_lines).replace("path: /api/v1/chat/completions", "path: /v1/chat/completions"),
                         encoding="utf-8")
            local_parsed = HermesHarness.parse_agent_trace(episode_dir)
        self.assertEqual(local_parsed["capture"]["semantic_events"], parsed["capture"]["semantic_events"])
        self.assertEqual([e for e in local_parsed["events"]
                          if e["type"] in {"reasoning", "tool_call", "tool_result", "assistant_text"}], semantic_events)
        ...
        agent_loop = {"schema_version":
                      ...
                      "events": [{"sequence": 1,
                                  ...
                                  "content": [{"type": "image_url",
                                               "image_url": {"url": "data:image/jpeg;base64,SECRETBYTES"}}]}, {"sequence": 2,
                                                                                                               "type": "model_response",
                                                                                                               "turn": 1,
    ...
    def test_hermes_trace_parser_pairs_concurrent_tools(self):
        trace = "\n".join(["hermes_chat_stdout:", "┌─ Reasoning ─────────┐", "I should search twice.",
                           "└─────────────────────┘",
                           "  ⚡ Concurrent: 2 tool calls — web_search, web_search", "  📞 Tool 1: web_search(['query'])",
                           "     Args: {\"query\": \"first\"}", "  📞 Tool 2: web_search(['query'])",
                           "     Args: {\"query\": \"second\"}", "  ┊ 🔍 search first", "  ✅ Tool 1 completed in 0.10s",
                           "     Result: {\"output\": \"first result\"}", "  ┊ 🔍 search second", "  ✅ Tool 2 completed in 0.20s",
                           "     Result: {\"output\": \"second result\"}", "hermes_chat_stderr:", "success: True"])
    ...
    def test_openclaw_trace_parser_standardizes_native_session(self):
        stdout = {"payloads": [{"text": "DONE"}],
                  "meta": {"durationMs": 50,
                           ...
                           "agentMeta": {"sessionId": "session-1",
                                         ...
                                         "usage": {"input": 10,
                                                   "output": 5}},
                           "systemPromptReport": {"systemPrompt": {"chars": 123,
                                                                   "hash": "prompt-hash"}}}}
        session = [{"type": "session",
                    "id": "session-1",
                    "cwd": "/workspace"}, {"type": "model_change",
                                           "provider": "openrouter",
                                           "modelId": "test-model"}, {"type": "thinking_level_change",
                                                                      "thinkingLevel": "high"}, {"type": "message",
                                                                                                 "message": {"role": "user",
                                                                                                             "content": [{"type": "text",
                                                                                                                          "text": "Play through MCP tools."}]}}, {"type": "message",
                                                                                                                                                                  "message": {"role":
                                                                                                                                                                              "assistant",
                                                                                                                                                                              "content": [{"type": "thinking",
                                                                                                                                                                                           "thinking": "I should search."}, {"type": "text",
                                                                                                                                                                                                                             "text": "I will verify this."}, {"type": "toolCall",
                                                                                                                                                                                                                                                              "id": "call-1",
                                                                                                                                                                                                                                                              "name": "web_search",
                                                                                                                                                                                                                                                              "arguments": {"query": "example"}}]}}, {"type": "message",
                                                                                                                                                                                                                                                                                                      "message": {"role": "toolResult",
                                                                                                                                                                                                                                                                                                                  "toolCallId": "call-1",
                                                                                                                                                                                                                                                                                                                  "toolName": "web_search",
                                                                                                                                                                                                                                                                                                                  "content": [{"type": "text",
                                                                                                                                                                                                                                                                                                                               "text": "search result"}]}}, {"type": "message",
                                                                                                                                                                                                                                                                                                                                                             "message": {"role": "assistant",
                                                                                                                                                                                                                                                                                                                                                                         "content": [{"type": "text",
                                                                                                                                                                                                                                                                                                                                                                                      "text": "DONE"}]}}]
        trajectory = [{"type": "context.compiled",
                       "data": {"systemPrompt": {"truncated": True,
                                                 "originalChars": 123},
                                "prompt": "Play through MCP tools.",
                                "tools": [{"name": "trajectory_tool"}]}}]
        request = {"messages": [{"role": "system",
                                 "content": "Native OpenClaw instruction."}, {"role": "user",
                                                                              "content": "Play through MCP tools."}],
                   "tools": [{"type": "function",
                              "function": {"name": "web_search"}}]}
        trace_lines = ["agent_loop_instruction_start", "Play through MCP tools.", "agent_loop_instruction_end",
                       "openclaw_agent_stdout:",
                       json.dumps(stdout), "openclaw_agent_stderr:", "openclaw_session: /tmp/session-1.jsonl",
                       *[json.dumps(record) for record in session], "openclaw_session: /tmp/session-1.trajectory.jsonl",
                       *[json.dumps(record)
                         for record in trajectory], "raw_upstream_request_1_start", "path: /api/v1/chat/completions",
                       json.dumps(request), "raw_upstream_request_1_end", "raw_upstream_response_1_start",
                       "path: /api/v1/chat/completions", "status: 200", "content_type: application/json",
                       "content_encoding: identity",
                       json.dumps({"choices": []}), "raw_upstream_response_1_end"]
        ...
        event_types = [event["type"] for event in parsed["events"]]
        instruction_kinds = [event.get("kind") for event in parsed["events"] if event["type"] == "instruction"]
    ...
    def test_openclaw_trace_parser_treats_post_game_abort_as_termination(self):
                                             ...
                                             "text": "Play through MCP tools."}]}}, {"type": "message",
                                                                                     "message": {"role":
                                                                                                 ...
                                                                                                 "content": [{"type": "toolCall",
                                                                                                              ...
                                                                                                              "arguments": {"response": "GUESS: answer"}}]}}, {"type": "message",
                                                                                                                                                               "message": {"role": "toolResult",
                                                                                                                                                                           "toolCallId": "call-1",
                                                                                                                                                                           "toolName": "clem_game__submit_response",
                                                                                                                                                                           "content": [{"type": "text",
                                                                                                                                                                                        "text": "done: true"}],
                                                                                                                                                                           "details": {"structuredContent": {"done": True}}}}, {"type": "custom",
                                                                                                                                                                                                                                "customType": "openclaw:prompt-error",
                                                                                                                                                                                                                                "data": {"error": "This operation was aborted | 20"}}, {"type": "message",
                                                                                                                                                                                                                                                                                        "message": {"role":
                                                                                                                                                                                                                                                                                                    "assistant",
                                                                                                                                                                                                                                                                                                    "stopReason":
                                                                                                                                                                                                                                                                                                    "aborted",
                                                                                                                                                                                                                                                                                                    "content": [{"type": "thinking",
                                                                                                                                                                                                                                                                                                                 "thinking": "Partial final reasoning."}]}}]
        trajectory = [{"type": "session.ended",
                       "data": {"status": "error",
                                "aborted": True,
                                "externalAbort": True,
                                "promptError": "This operation was aborted | 20"}}]
        trace_lines = ["openclaw_session: /tmp/session-1.jsonl", *[json.dumps(record) for record in session],
                       "openclaw_session: /tmp/session-1.trajectory.jsonl", *[json.dumps(record) for record in trajectory]]
        ...
        meaningful_events = [event for event in parsed["events"] if event["type"] in {"reasoning", "termination", "error"}]
        self.assertEqual([event["type"] for event in meaningful_events], ["reasoning", "termination"])
        self.assertEqual(parsed["result"]["status"], "completed")
        self.assertTrue(parsed["result"]["success"])
        self.assertEqual(parsed["result"]["terminal_reason"], "terminated_after_game_completion")
    ...
    def test_openclaw_trace_parser_recovers_partial_wire_history(self):
        ...
        search_result = {"role": "tool", "tool_call_id": "call-1", "content": "search result"}
        submit_call = {"role":
                       "assistant",
                       "content":
                       None,
                       ...
                       "tool_calls": [{"id": "call-2",
                                       "type": "function",
                                       "function": {"name": "clem_game__submit_response",
                                                    "arguments": json.dumps({"response": "GUESS: answer"})}}]}
        submit_result = {"role": "tool", "tool_call_id": "call-2", "content": json.dumps({"done": True})}
        requests = [[system_message, user_message], [system_message, user_message, search_call, search_result],
                    [system_message, user_message, search_call, search_result, submit_call, submit_result]]
        trace_lines = ["agent_loop_instruction_start", instruction, "agent_loop_instruction_end"]

        for request_number, messages in enumerate(requests, start=1):
            trace_lines.extend([f"raw_upstream_request_{request_number}_start", "path: /api/v1/chat/completions",
                                json.dumps({"messages": messages}), f"raw_upstream_request_{request_number}_end"])
        ...
        self.assertEqual([event["name"] for event in calls], ["web_search", "clem_game__submit_response"])
    ...
    def test_bridge_exposes_start_and_submit_without_get_state(self):
        ...
        with patch("clemagents.mcp.bridge.OpenEnvMCPClient", return_value=client):
            bridge = create_mcp_bridge("http://example.invalid/mcp")

        tools = asyncio.run(bridge.list_tools())

        self.assertEqual([tool.name for tool in tools], ["start_game", "submit_response"])

    def test_bridge_aborts_repeated_start_game(self):
        client = unittest.mock.MagicMock()
        client.call_tool.side_effect = [{"context": {"role": "user",
                                                     "content": "Initial game message"},
                                         "reward": None,
                                         "done": False,
                                         "metadata": {}}, {"context": {"role": "user",
                                                                       "content": "Initial game message"},
                                                           "reward": -1.0,
                                                           "done": False,
                                                           "metadata": {}}]
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("clemagents.mcp.bridge.OpenEnvMCPClient",
                       return_value=client), patch.dict("os.environ", {"GAME_COMPLETION_PATH": str(completion_path)}):
                bridge = create_mcp_bridge("http://example.invalid/mcp")
        ...
        self.assertEqual(result.structured_content, {"message": REPEATED_START_MESSAGE})
        self.assertEqual(json.loads(result.content[0].text), {"message": REPEATED_START_MESSAGE})
        self.assertEqual(client.call_tool.call_args_list, [unittest.mock.call("start_game", {}),
                                                           unittest.mock.call("submit_response", {"response": CONTROL_FAILURE_RESPONSE})])
    ...
    def test_bridge_aborts_submit_response_before_start_game(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("clemagents.mcp.bridge.OpenEnvMCPClient",
                       return_value=client), patch.dict("os.environ", {"GAME_COMPLETION_PATH": str(completion_path)}):
                bridge = create_mcp_bridge("http://example.invalid/mcp")
                result = asyncio.run(bridge.call_tool("submit_response", {"response": "guess"}))

            completion = json.loads(completion_path.read_text(encoding="utf-8"))

        self.assertEqual(result.structured_content, {"message": ("The episode was aborted because submit_response was called "
                                                                 "before start_game.")})
        self.assertEqual(json.loads(result.content[0].text), {"message": ("The episode was aborted because submit_response was called "
                                                                          "before start_game.")})
    ...
    def test_bridge_writes_completion_marker(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("clemagents.mcp.bridge.OpenEnvMCPClient",
                ...
                async def finish_game():
                    ...
                    final_result = await bridge.call_tool("submit_response", {"response": "guess"})
        ...
        self.assertEqual(start_result.structured_content, {"message": "Initial game message"})
        self.assertEqual(json.loads(start_result.content[0].text), {"message": "Initial game message"})
        self.assertEqual(final_result.structured_content, {"message": "Final game message"})
        self.assertEqual(json.loads(final_result.content[0].text), {"message": "Final game message"})

    def test_bridge_preserves_text_format_and_adds_mcp_image_content(self):
        client = unittest.mock.MagicMock()
        client.call_tool.return_value = {"context": {"role": "user",
                                                     "content": "Identify this location.",
                                                     "image": [{"data": "aW1hZ2UtYnl0ZXM=",
                                                                "mimeType": "image/jpeg"}]},
                                         "reward": None,
                                         "done": False,
                                         "metadata": {}}
        ...
        self.assertEqual(json.loads(result.content[0].text), {"message": "Identify this location."})
        self.assertEqual(result.content[1].type, "image")
        self.assertEqual(result.content[1].data, "aW1hZ2UtYnl0ZXM=")
        self.assertEqual(result.content[1].mimeType, "image/jpeg")
        self.assertEqual(result.structured_content, {"message": "Identify this location."})

    def test_bridge_materializes_only_sequential_observation_images(self):
        client = unittest.mock.MagicMock()
        client.call_tool.side_effect = [{"context": {"role": "user",
                                                     "content": "First view.",
                                                     "image": [{"data": "Zmlyc3QtaW1hZ2U=",
                                                                "mimeType": "image/jpeg"}]},
                                         "reward": None,
                                         "done": False,
                                         "metadata": {}}, {"context": {"role": "user",
                                                                       "content": "Second view.",
                                                                       "image": [{"data": "c2Vjb25kLWltYWdl",
                                                                                  "mimeType": "image/png"}]},
                                                           "reward": None,
                                                           "done": False,
                                                           "metadata": {}}]
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("clemagents.mcp.bridge.OpenEnvMCPClient",
                       return_value=client), patch.dict("os.environ", {"GAME_OBSERVATION_DIR": str(observation_dir)}):
                bridge = create_mcp_bridge("http://example.invalid/mcp")

                async def advance_game():
                    first = await bridge.call_tool("start_game", {})
                    second = await bridge.call_tool("submit_response", {"response": "next"})
                    return first, second
            ...
            self.assertNotIn("resource_link", [block.type for block in first_result.content])
            ...
            self.assertNotIn("resource_link", [block.type for block in second_result.content])
    ...
    def test_hermes_timeout_is_a_failed_episode_not_an_exception(self):
        ...
        self.assertEqual(result.metadata["runtime_error"], "Hermes timed out after 1s")
    ...
    def test_hermes_cli_exit_before_game_done_is_not_success(self):
        ...
        self.assertEqual(result.metadata["runtime_error"], "Hermes ended before clem_game reported done=true")
    ...
    def test_hermes_records_both_provider_routes_without_verbose_printer(self):
        ...
        payload = {"model":
                   ...
                   "messages": [{"role":
                                 "user",
                                 "content": [{"type": "text",
                                              "text": "Inspect this image."}, {"type": "image_url",
                                                                               "image_url": {"url": "data:image/png;base64,ZmFrZQ=="}}]}],
                   "tools": [{"type": "function",
                              "function": {"name": "mcp__game__submit_response",
                                           "parameters": {"type": "object",
                                                          "properties": {}}}}]}

        try:
            for provider, backend, endpoint_variable in (("openai-api", "openai_compatible", "OPENAI_BASE_URL"),
                ...
                with self.subTest(provider=provider), tempfile.TemporaryDirectory() as directory:
                    ...
                    def setup(command, **kwargs):
                        config.update(yaml.safe_load((Path(os.environ["HERMES_HOME"]) / "config.yaml").read_text()))
                        return subprocess.CompletedProcess(command, 0, "  start_game tool\n  submit_response tool\n", "")
                    ...
                    def chat(command, **kwargs):
                        ...
                        self.assertTrue(config["display"]["show_reasoning"])
                        ...
                        observe("pre_api_request", request={"body": payload}, api_request_id="one")
                        response = requests.post(f"{endpoint}/chat/completions", json=payload, timeout=5)
                        response.raise_for_status()
                        self.assertEqual(response.json(), response_body)
                        observe("post_api_request", assistant_message=response_body["choices"][0]["message"],
                                api_request_id="one")
                    ...
                    native_trace = (Path(directory) / "hermes_observer.jsonl").read_text()
                    self.assertIn("pre_api_request", native_trace)
                    self.assertIn("post_api_request", native_trace)
                    (Path(directory) / "agent_trace.log").write_text(trace, encoding="utf-8")
                    parsed = HermesHarness.parse_agent_trace(Path(directory))
                    self.assertEqual(parsed["capture"]["model_requests"]["count"], 1)
                    self.assertEqual(parsed["capture"]["model_responses"]["count"], 1)
                    self.assertEqual(parsed["capture"]["tool_definitions"]["status"], "partial")
        finally:
            upstream.shutdown()
    ...
    def test_closed_openenv_session_removes_recovery_marker(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            session_path.write_text(json.dumps({"session_id": "session-1"}), encoding="utf-8")
            client = OpenEnvMCPClient("http://example.invalid/mcp")
            client.session_id = "session-1"

            with patch.object(client, "_request",
                              return_value={}) as request, patch.dict("os.environ",
                                                                      {"GAME_SESSION_PATH": str(session_path)}):
                client.close_session()

            request.assert_called_once_with("openenv/session/close", {"session_id": "session-1"})
            self.assertIsNone(client.session_id)
            self.assertFalse(session_path.exists())

    def test_openclaw_rejects_mismatched_openrouter_connection(self):
        with self.assertRaisesRegex(ValueError, "canonical"):
            _validate_openclaw_model_connection({"backend": "openrouter",
    ...
    def test_openclaw_openrouter_resolution_uses_canonical_model(self):
        model_spec = {"model_name": "gpt-5-mini-openrouter", "backend": "openrouter", "model_id": "openai/gpt-5-mini"}
        with patch.object(model_connection, "_find_model_spec",
                          return_value=model_spec), patch.object(model_connection, "_openrouter_key_config",
                                                                 return_value={"api_key": "openrouter-test-key"}):
            connection = OpenClawHarness.resolve_model_connection(model_spec, {})

        self.assertEqual(connection["harness"], "openclaw")
        self.assertEqual(connection["backend"], "openrouter")
        self.assertEqual(connection["model"], "openrouter/openai/gpt-5-mini")
        self.assertNotIn("upstream_model", connection)
        self.assertEqual(connection["base_url"], "https://openrouter.ai/api/v1")
        self.assertEqual(connection["openclaw_provider"], "openrouter")
        self.assertEqual(connection["env"], {"OPENROUTER_API_KEY": "openrouter-test-key"})

    def test_openclaw_openai_compatible_resolution_uses_registry_modalities(self):
        model_spec = {"model_name": "multimodal-test-model",
                      "backend": "openai_compatible",
                      "model_id": "local/multimodal-test-model",
                      "context_size": "128k",
                      "model_config": {"input_modalities": ["text", "image", "audio"],
                                       "extra_body": {"reasoning": {"effort": "high"}}}}
        key_config = {"api_key": "local-test-key", "base_url": "http://localhost:11434/v1"}

        with patch.object(model_connection, "_find_model_spec",
                          return_value=model_spec), patch.object(model_connection, "_openai_compatible_key_config",
                                                                 return_value=key_config):
            connection = OpenClawHarness.resolve_model_connection(model_spec, {})

        provider = connection["openclaw_config_patch"]["models"]["providers"]["openai_compatible"]
    ...
    def test_openclaw_openai_compatible_resolution_defaults_to_text_input(self):
        model_spec = {"model_name": "text-test-model",
                      "backend": "openai_compatible",
    ...
    def test_all_harnesses_resolve_openai_compatible_models(self):
        ...
        with patch.object(model_connection, "_find_model_spec",
                          return_value=model_spec), patch.object(model_connection, "_openai_compatible_key_config",
                                                                 return_value=key_config):
            connections = {"claude_code": ClaudeCodeHarness.resolve_model_connection(model_spec, {}),
                           "codex": CodexHarness.resolve_model_connection(model_spec, {}),
                           "hermes": HermesHarness.resolve_model_connection(model_spec, {}),
                           "openclaw": OpenClawHarness.resolve_model_connection(model_spec, {})}
        ...
        for harness, connection in connections.items():
            ...
            self.assertNotIn("request_body_overrides", connection)

        self.assertEqual(connections["hermes"]["provider"], "openai-api")
        self.assertNotIn("runtime_model", connections["claude_code"])
        self.assertEqual(connections["claude_code"]["model"], "Qwen/Qwen-test")
        self.assertEqual(connections["openclaw"]["openclaw_provider"], "openai_compatible")
    ...
    def test_agent_reasoning_off_disables_provider_reasoning(self):
        for effort in ("none", "off"):
            with self.subTest(effort=effort), tempfile.TemporaryDirectory() as directory:
                registry_path = Path(directory) / "agent_registry.json"
                registry_path.write_text(json.dumps([{"agent_name": "test-agent",
                                                      "backend": "codex",
                                                      "agent_config": {"clem_model": "test-model",
                                                                       "reasoning_effort": effort}}]), encoding="utf-8")

                with patch.object(model_connection, "_find_model_spec", return_value={"model_name": "test-model",
                                                                                      "model_id": "provider/test-model",
                                                                                      "backend": "openrouter",
                                                                                      "model_config": {"extra_body": {"reasoning": {"enabled": True,
                                                                                                                                    "effort": "high"}}}}), patch.object(model_connection, "_openrouter_key_config",
                                                                            return_value={"api_key": "test-key"}):
                    connection = model_connection.resolve_agent_model_connection("test-agent", registry_path)

                self.assertNotIn("request_body_overrides", connection)

    def test_proxy_rejects_legacy_overrides(self):
        for key, value in (("request_body_overrides", {"reasoning": {"enabled": False}}),
                           ("generation_overrides", {"temperature": 1}), ("upstream_model", "other-model")):
            with self.subTest(key=key), self.assertRaisesRegex(ValueError, key):
                proxy_for_model_connection({"backend": "openai_compatible", "base_url": "http://localhost/v1",
                                            key: value}, Path("/tmp/not-used"))
        ...
        try:
            with tempfile.TemporaryDirectory() as directory:
                ...
                with OpenAICompatibleProxy(target, completion_path) as proxy:
                    openai_body = {"model": "qwen",
                                   "tools": [{"type": "function",
                                              "function": {"name": "start_game"}}],
                                   "tool_choice": "auto"}
                    requests.post(f"{proxy.base_url}/chat/completions", json=openai_body, timeout=5).raise_for_status()
                    requests.post(f"{proxy.base_url}/messages", json={"model": "qwen",
                                                                      "tools": openai_body["tools"]}, timeout=5).raise_for_status()

                    completion_path.write_text('{"done":true}', encoding="utf-8")
                    post_game_response = requests.post(f"{proxy.base_url}/chat/completions", json=openai_body,
                                                       timeout=5)
        finally:
            upstream.shutdown()
            upstream.server_close()
    ...
    def test_proxy_forwards_exact_bodies_and_records_them(self):
        received = []
        response_body = b'data: {"type":"response.completed", "response":{"output":[]}}\n\ndata: [DONE]\n\n'
        ...
        thread.start()
        body = (b'{ "model": "native/subagent-model", "temperature": 0.7, "reasoning": {"effort":"low"},'
                b' "thinking":{"type":"adaptive"}, "output_config":{"effort":"low"},'
                b' "input":[{"type":"input_image","image_url":"data:image/png;base64,ZmFrZQ=="}],'
                b' "tools":[{"type":"namespace","name":"mcp__game","tools":[{"type":"function","name":"start_game"}]}]}')
        try:
            with tempfile.TemporaryDirectory() as directory:
                with OpenAICompatibleProxy(f"http://127.0.0.1:{upstream.server_port}/v1",
                                           Path(directory) / "done.json", trace_requests=True) as proxy:
                    for path in ("responses", "chat/completions", "messages"):
                        response = requests.post(f"{proxy.base_url}/{path}", data=body, timeout=5)
                        self.assertEqual(response.content, response_body)
                        self.assertEqual(received[-1], (f"/v1/{path}", body))
                    self.assertIn(body.decode(), proxy.captured_trace())
                    self.assertIn(response_body.decode(), proxy.captured_trace())
                    for raw in (b"invalid json", b"[]", b"null", b""):
                        requests.post(f"{proxy.base_url}/responses", data=raw, timeout=5).raise_for_status()
                        self.assertEqual(received[-1][1], raw)
        finally:
            upstream.shutdown()
            upstream.server_close()
            thread.join(timeout=5)

    def test_observer_does_not_change_body_and_still_detects_completion(self):
        body = b'{ "input": [{"type":"function_call_output","output":"{\\"done\\":true}"}]}'
        with tempfile.TemporaryDirectory() as directory:
            marker = Path(directory) / "done.json"
            _observe_request_body(body, marker)
            self.assertTrue(marker.exists())
            self.assertTrue(body.startswith(b'{ "input"'))
    ...
    def test_proxy_blocks_new_inference_after_game_completion(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as directory:
                ...
                with OpenAICompatibleProxy(target, completion_path) as proxy:
                    response = requests.post(f"{proxy.base_url}/chat/completions", json={"model": "test",
                                                                                         "messages": []}, timeout=5)
    ...
    def test_process_is_terminated_after_game_completion(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result, terminated = run_process_until_game_complete([sys.executable, "-c", script],
                                                                 completion_path=completion_path, completion_grace=0.01,
                                                                 timeout=5)
    ...
    def test_host_waits_for_artifacts_ready_after_game_completion(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            script = "\n".join(["import json, time", "from pathlib import Path",
                                f"completion = Path({str(completion_path)!r})",
                                f"ready = Path({str(ready_path)!r})", f"artifacts = Path({str(artifact_dir)!r})",
                                "completion.write_text(json.dumps({'done': True, 'source': 'test'}))", "print('game done', flush=True)",
                                "time.sleep(0.25)", "artifacts.mkdir(parents=True)",
                                "(artifacts / 'native_session.jsonl').write_text('native artifact')", "ready.write_text(json.dumps({",
                                "    'ready': True,", "    'status': 'complete',",
                                "    'artifact_directory': 'artifacts/test-agent/run-1',", "}))",
                                "print('artifacts ready', flush=True)", "time.sleep(0.05)"])
            actual_popen = subprocess.Popen
            ...
            with patch("clemagents.run_pipeline.utils.KEYS_PATH",
                       keys_path), patch("clemagents.run_pipeline.utils.REGISTRY_PATH",
                                         registry_path), patch("clemagents.run_pipeline.utils.subprocess.Popen",
                                                               side_effect=simulated_container):
                trace, returncode, metadata = run_docker_episode(experiment_name="test", game_id=1,
                                                                 agent_name="test-agent", shared_state_dir=shared_dir,
                                                                 episode_timeout=5, artifact_finalization_timeout=2,
            ...
            self.assertEqual(metadata["agent_artifact_relative_path"], "artifacts/test-agent/run-1")
            self.assertLess(metadata["episode_elapsed_seconds"], metadata["container_elapsed_seconds"])
    ...
    def test_host_bounds_artifact_finalization_after_game_completion(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            script = "\n".join(["import json, time", "from pathlib import Path",
                                ...
                                "print('game done', flush=True)", "time.sleep(30)"])
            actual_popen = subprocess.Popen
            ...
            with patch("clemagents.run_pipeline.utils.KEYS_PATH",
                       keys_path), patch("clemagents.run_pipeline.utils.REGISTRY_PATH", registry_path
                                         ), patch("clemagents.run_pipeline.utils.subprocess.Popen",
                                                  side_effect=simulated_container
                                                  ), patch("clemagents.run_pipeline.utils._stop_docker_container",
                                                           side_effect=stop_simulated_container):
                trace, returncode, metadata = run_docker_episode(experiment_name="test", game_id=1,
                                                                 agent_name="test-agent", shared_state_dir=shared_dir,
                                                                 episode_timeout=5, artifact_finalization_timeout=0.15,
    ...
    def test_openclaw_passes_openrouter_key_only_through_process_environment(self):
        ...
        def fake_agent_run(command, **kwargs):
            ...
            observed_api_keys.append(os.environ.get("OPENROUTER_API_KEY"))
            kwargs["completion_path"].write_text('{"done":true,"control_failure":false}', encoding="utf-8")
        ...
        with tempfile.TemporaryDirectory(
            ...
            result = OpenClawHarness(reasoning_effort="medium", temperature=0.8,
                                     model_connection_path="ignored").run_episode("Play the game.",
                                                                                  output_dir=directory)
            gateway.assert_called_once()

        config_input = next(input_text for command, input_text in calls if "config" in command and "patch" in command)
        config = json.loads(config_input)
        self.assertNotIn("env", config)
        self.assertTrue(observed_api_keys)
        self.assertTrue(all(key == "openrouter-test-key" for key in observed_api_keys))
        self.assertTrue(config["models"]["providers"]["openrouter"]["baseUrl"].startswith("http://127.0.0.1:"))
        self.assertEqual(config["plugins"], {"enabled": True, "allow": ["openrouter", "duckduckgo", "browser"],
                                            "entries": {"perplexity": {"enabled": False}}})
        self.assertEqual(config["browser"], {"enabled": True,
                                             "headless": True,
                                             "noSandbox": True,
                                             "executablePath": "/usr/bin/chromium",
                                             "defaultProfile": "openclaw"})
        self.assertEqual(config["gateway"]["bind"], "loopback")
        self.assertEqual(config["gateway"]["auth"], {"mode": "token", "token": "${OPENCLAW_GATEWAY_TOKEN}"})
        self.assertEqual(config["agents"]["defaults"]["heartbeat"]["every"], "0m")
        self.assertEqual(config["agents"]["defaults"]["model"]["primary"], "openrouter/openai/gpt-5-mini")
        self.assertEqual(config["agents"]["defaults"]["models"]["openrouter/openai/gpt-5-mini"]["params"],
                         {"temperature": 0.8})
        ...
        self.assertEqual(config["tools"]["web"]["search"], {"enabled": True, "provider": "duckduckgo"})
        self.assertEqual(config["tools"]["exec"], {"security": "full", "ask": "off"})
        agent_command = next(command for command, _input_text in calls if "agent" in command)
        self.assertNotIn("--local", agent_command)
        self.assertTrue(agent_command[agent_command.index("--session-key") + 1].startswith("agent:main:game-session-"))
        self.assertEqual(agent_command[agent_command.index("--thinking") + 1], "medium")
        ...
        for agent_fails in (False, True):
            with self.subTest(agent_fails=agent_fails), tempfile.TemporaryDirectory() as directory:
                ...
                with patch("clemagents.adapters.openclaw.openclaw.subprocess.Popen",
                    ...
                    self.assertEqual(popen.call_args.args[0], ["openclaw", "--profile", "isolated", "gateway", "run"])
    ...
    def test_result_run_dir_orders_openclaw_and_environment_players(self):
        with tempfile.TemporaryDirectory() as directory:
            registry_path = Path(directory) / "agent_registry.json"
            registry_path.write_text(json.dumps([{"agent_name": "openclaw-with-gpt-5-mini-openrouter",
                                                  "backend": "openclaw",
                                                  "agent_config": {"clem_model": "gpt-5-mini-openrouter"}}]), encoding="utf-8")

            run_dir = result_run_dir_name(agent_name="openclaw-with-gpt-5-mini-openrouter", registry_path=registry_path,
                                          learner_agent="player_0", env_agents={"player_1": "Llama-4-Maverick"})

        self.assertEqual(run_dir, "openclaw-with-gpt-5-mini-openrouter--Llama-4-Maverick")

    def test_failed_trace_never_reuses_another_harness_directory(self):
        with tempfile.TemporaryDirectory() as directory:
            results_dir = Path(directory)
            wrong_episode = (results_dir / "hermes-with-gpt-5-mini-openrouter--Llama-4-Maverick" / "taboo" / "high_en" /
                             "episode_00001")
            wrong_episode.mkdir(parents=True)
            (wrong_episode / "instance.json").write_text(json.dumps({"game_id": 0}), encoding="utf-8")

            run_dir = ("openclaw-with-gpt-5-mini-openrouter--Llama-4-Maverick")
            trace_path = write_agent_artifacts(results_dir=str(results_dir), run_dir=run_dir, game_name="taboo",
                                               experiment_name="high_en", game_id=0, before_instance_dirs=set(),
                                               trace_text="openclaw failure",
                                               metadata={"started_at": datetime.now(timezone.utc).isoformat()})

            self.assertEqual(trace_path.read_text(encoding="utf-8"), "openclaw failure")
            self.assertTrue(trace_path.is_relative_to(results_dir / "_agent_failures" / run_dir))
