...
class TestOpenClawNativeStartup(unittest.TestCase):
    ...
    def test_gateway_agent_reaches_recorded_api_and_preserves_artifacts(self):
                ...
                chunk = {"id":
                         ...
                         "choices": [{"index": 0,
                                      "delta": {"role": "assistant",
                                                "content": "OFFLINE_FIXTURE_COMPLETE"},
                                      "finish_reason": None}]}
        ...
        connection = {"backend": "openai_compatible",
                      "model": "fixture",
                      "runtime_model": "offline/fixture",
                      "base_url": base_url,
                      "openclaw_provider": "offline",
                      "env": {},
                      "openclaw_config_patch": {"models": {"providers": {"offline": {"baseUrl":
                                                                                     base_url,
                                                                                     "apiKey":
                                                                                     "offline-test-not-a-secret",
                                                                                     "api":
                                                                                     "openai-completions",
                                                                                     "models": [{"id": "fixture",
            ...
            with tempfile.TemporaryDirectory() as directory, patch("clemagents.adapters.openclaw.load_model_connection",
                ...
                with redirect_stdout(io.StringIO()):
                    result = OpenClawHarness(model_connection_path="unused",
                                             thinking="off").run_episode("Offline startup fixture",
                ...
                self.assertTrue(any("start_game" in tool.get("function", {}).get("name", "")
                                    for tool in requests[0].get("tools", [])), "the native MCP inventory must reach the API")
                ...
                self.assertTrue(any(path.suffix in {".jsonl", ".sqlite"} for path in native_files))
